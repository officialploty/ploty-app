  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '18px 18px 12px' }}>
        <div style={{ font: '800 24px/1 Manrope', color: '#1a1e1c', letterSpacing: '-.02em' }}>{list.length} saved plot{list.length === 1 ? '' : 's'}</div>
      </div>
      <div className="pmScroll" style={{ flex: 1, overflowY: 'auto', padding: '0 18px 30px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {list.length === 0 && (
          <div style={{ padding: '40px 10px', textAlign: 'center', font: '500 13px/1.6 Manrope', color: '#8a958f' }}>Tap the ♥ on any plot to save it here.</div>
        )}
      </div>
    </div>
  );
